...
def build_model():
    ...
    if model_name.startswith("openai:"):
        ...
        return OpenAIModel(cast(OpenAIModelName,model_name[7:]),provider=OpenAIProvider(api_key=api_key),model_settings=model_settings)
    
    elif model_name.startswith("google-gla:"):
        from pydantic_ai.models.gemini import GeminiModel,GeminiModelName
        from pydantic_ai.providers.google_gla import GoogleGLAProvider

        return GeminiModel(cast(GeminiModelName,model_name[11:]),provider=GoogleGLAProvider(api_key=api_key),model_settings=model_settings)

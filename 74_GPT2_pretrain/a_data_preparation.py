...
if __name__=='__main__':
    ...
    dataset=load_dataset('bookcorpus',trust_remote_code=True,split='train').shuffle(seed=42).select(range(5000))
    dataset=dataset.train_test_split(test_size=0.25)
    print('after train split',dataset)
    ...
    print('after tokenizing:',tokenized_data)
    print(">>>> Tokenization End")

    # make samples to size of context window...
    print(">>>> beigining of  chunkking of tokenized data:")
    concated_tokens_data=tokenized_data.map(concat,batched=True,batch_size=1000,num_proc=8)
    print('after concatenatiung the tokens data:',concated_tokens_data)
    ...
    print('after chunking',chunked_ds['train'][0]['input_ids'])
    print('after chunking',chunked_ds['train'][1]['input_ids'])
